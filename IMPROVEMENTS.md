...
- **Plus de commandes dans le mode commande** : seuls `o` et `Maj+O` existent aujourd'hui ; `h`/`j`/`k`/`l`, `x`, `dd`, `yy`/`p`, `u`, `gg`/`G`, `/` pour chercher, voire `:w` / `:q`, en feraient un vrai mode Vim. Les touches non gérées sont simplement ignorées.
