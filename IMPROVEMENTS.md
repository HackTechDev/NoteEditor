...
- **Plus de commandes dans le mode commande** : seuls `o`, `Maj+J`, `0`, `$` et `u` existent aujourd'hui ; `O` (ligne au-dessus), `^` (premier caractère non blanc), `h`/`j`/`k`/`l`, `x`, `dd`, `yy`/`p`, `Ctrl+R` (rétablir), `gg`/`G`, `/` pour chercher, voire `:w` / `:q`, en feraient un vrai mode Vim. Les touches non gérées sont simplement ignorées.
...
- **Aperçu Markdown plus fin** : le défilement, désormais synchronisé dans les deux sens, reste proportionnel (position relative dans le document) ; un calage sur la ligne réellement affichée serait plus précis pour les longues notes avec images ou tableaux, dont les proportions diffèrent entre le texte brut et le rendu. L'export HTML pourrait aussi produire un HTML plus léger, avec une feuille de style propre à la place des styles en ligne de Qt (au lieu de continuer à les compléter après coup, comme les quelques fonds et bordures ajoutés à l'aperçu et à l'export), ou un export PDF.
- **Coloration syntaxique dans les blocs de code de l'aperçu Markdown** : contrairement à l'éditeur, un bloc ```` ```python ```` ne colore pas le code, quel que soit le langage indiqué après les ```` ``` ```` — tout le bloc reste d'une seule couleur.
