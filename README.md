...
- Aperçu Markdown en volet séparé pour les fichiers `.md` / `.markdown` (icône de la barre d'outils, désactivée pour les autres onglets) : le rendu, mis à jour en direct pendant la frappe et mis en forme (fonds et police à chasse fixe pour le code, fonds pour les citations, bordures pour les tableaux et les titres), s'affiche à droite de l'éditeur et son défilement est synchronisé avec celui de l'éditeur dans les deux sens ; « Fichier → Exporter en HTML... » enregistre le rendu, avec la même mise en forme, dans une page HTML autonome
...
- Menu Fichier → « Notes fermées récemment » : les dernières notes fermées, 10 par défaut et réglable de 1 à 50 (y compris les fichiers extérieurs qui ne figurent plus dans le panneau), à rouvrir en un clic ; la liste est mémorisée d'un lancement à l'autre ; une note vide sans fichier n'y figure que si l'on coche « Mémoriser à la fermeture » dans son menu contextuel
...
- Panneau « Brouillons » à gauche (que l'icône de la barre d'outils masque ou rend, choix mémorisé) : liste les notes archivées dans `~/.noteeditor` (ouvertes marquées « (ouvert) »), avec recherche (par nom ou par contenu des notes), tri (date/nom), l'entrée de l'onglet actif surlignée et, au survol, une infobulle indiquant le chemin du fichier ; double-clic pour rouvrir ou basculer dessus, clic droit pour la même palette d'actions que le menu contextuel des onglets (fermer / fermer les autres / fermer à droite / fermer tout, épingler, mémoriser à la fermeture, dupliquer, historique des versions, copier le nom ou le chemin complet du fichier, ouvrir son dossier), renommer (onglets sans fichier) ou mettre à la corbeille
- Fichiers extérieurs à `~/.noteeditor` (ouverts avec Ouvrir) : une fois fermés, ils ne figurent plus dans le panneau Brouillons (leur texte est dans le fichier lui-même), avec une alerte « Enregistrer / Ne pas enregistrer / Annuler » s'ils ont des modifications non enregistrées ; ils ne peuvent pas être mis à la corbeille (l'entrée est grisée, seule la fermeture les retire du panneau) ; le panneau permet aussi, par sélection multiple, de fermer, épingler, détacher ou mettre à la corbeille plusieurs notes d'un coup (les fichiers extérieurs de la sélection sont ignorés pour cette dernière action)
...
| Joindre la ligne à la suivante (mode commande) | Shift+J |
| Début de ligne (mode commande) | 0 |
| Fin de ligne (mode commande) | $ |
| Annuler (mode commande) | u |
...
- `~/.noteeditor/session.json` : liste des onglets actuellement ouverts (fichier associé, nom par défaut, état modifié, position du curseur, sélection et défilement, onglet actif)
